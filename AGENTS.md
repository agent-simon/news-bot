...
- Edit the ignored root-level `sources.local.json` for local feeds/topics; `sources.shadow.json` is the empty tracked template. Do not edit or commit `seen_links.db`/`seen_links.json`; they are runtime dedup state.
...
- `config.py` reads environment flags and the feed/topic JSON on each call; `SOURCES_PATH` overrides the default root-level `sources.local.json`.
