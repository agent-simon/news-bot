...
- **news.py** — Core logic:
  - `load_config()`: reads the feed/topic config **fresh on each run** so config edits take effect without restarting the bot. Path is `SOURCES_PATH` if set, else the module-relative `sources.json` (CWD-independent). `THEMES_PER_RUN` (how many themes `/news` samples per run) stays in `news.py`.
